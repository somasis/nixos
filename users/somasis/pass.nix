...
in
{
  ...
  cache.directories =
    lib.optional (config.programs.dmenu-pass.enable) {
      method = "symlink";
      directory = config.lib.somasis.xdgCacheDir "dmenu-pass";
    };

  # Necessary only because `pass` uses GPG keys... could be removed if I used something `age` based
  ...
  programs.password-store = {
    ...
    package = (pkgs.pass.override { dmenu = config.programs.dmenu.package; }).withExtensions (exts: with exts; [
      pass-checkup
      pass-update
  ...
  programs.dmenu-pass = {
    enable = config.programs.dmenu.enable;
